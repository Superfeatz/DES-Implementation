...
def circularShiftLeft(binaries, shift):
    cut = binaries[:shift]
...
def initialPermutation(binaries):
...
def hex_to_bin(hex_string):
    ...
    return ''.join(format(int(hex_string[i:i+2], 16), '08b') for i in range(0, len(hex_string), 2))

def bin_to_hex(bin_string):
    """Convert binary string to hex"""
    return ''.join(format(int(bin_string[i:i+8], 2), '02x') for i in range(0, len(bin_string), 8))
...
def DES_decrypt(ciphertext_bin):
    """Decrypt using binary ciphertext"""
    binaries = ciphertext_bin
    binaries = initialPermutation(binaries)
    left = binaries[:32]
    right = binaries[32:]
    for i in range(15, -1, -1):  # Reverse the key order for decryption
        temp_right = right
        right = feistel(right, i)
        right2 = ''
        for j in range(len(right)):
...
if choice == '1':
    ...
    for i in range(0, len(plaintext), 8):
        ...
        encrypted_block = DES_encrypt(block)
        ciphertext_bin += encrypted_block

    ciphertext_hex = bin_to_hex(ciphertext_bin)
    print("Ciphertext hex encoded: " + ciphertext_hex)

elif choice == '2':
    print("Input ciphertext (hex encoded): ")
    ...
    decrypted_text = ''
    for i in range(0, len(ciphertext_bin), 64):  # 64-bit blocks
        block = ciphertext_bin[i:i+64]
        decrypted_block = DES_decrypt(block)
        for j in range(0, len(decrypted_block), 8):
            decrypted_text += chr(int(decrypted_block[j:j+8], 2))
